...
(defun pick-chance-branch (board move) ;; it called when players select
  (labels ((dice (pos)
	     (cadr (aref board pos))))
    (let ((path (car move)))
      (if (or (null path) (roll-against (dice (car path)) ;; not path-nil -> attack phase ;; path-nil -> pass
					(dice (cadr path))))
	  (cadr move) ;; attack Success ;; pass turn
	  (caddr move))))) ;; attack Fail
...
(defparameter *dice-probability* #(#(0.84 0.97 1.0 1.0) ;; column : enemy's dice ;; line : player's dice
				   #(0.44 0.78 0.94 0.99)
				   #(0.15 0.45 0.74 0.91)
				   #(0.04 0.19 0.46 0.72)
				   #(0.01 0.06 0.22 0.46)))
